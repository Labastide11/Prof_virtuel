...
(function(){
  ...
  function injectStyle(){
    if($('hibouVocabulaireV25725Style')) return;
    var css = ''+
    '.vocab-v25725-menu{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:12px}'+
    '.vocab-v25725-card{border:2px solid #ddd6fe;background:linear-gradient(135deg,#faf5ff,#ffffff);border-radius:22px;padding:18px 14px;text-align:center;font-family:inherit;font-weight:900;color:#4c1d95;cursor:pointer;box-shadow:0 8px 24px rgba(124,58,237,.12)}'+
    '.vocab-v25725-card:hover{transform:translateY(-1px);box-shadow:0 12px 28px rgba(124,58,237,.16)}'+
    '.vocab-v25725-icon{font-size:42px;margin-bottom:8px}.vocab-v25725-title{font-size:20px;color:#4c1d95}.vocab-v25725-desc{font-size:13px;color:#6d28d9;margin-top:5px;line-height:1.25}'+
    '.vocab-v25725-box{background:#fff;border:2px solid #e9d5ff;border-radius:18px;padding:16px;margin-top:12px}'+
    '.vocab-v25725-question{background:#faf5ff;border:2px solid #ddd6fe;border-radius:16px;padding:14px;font-size:20px;font-weight:1000;color:#2e1065;margin:10px 0;line-height:1.3}'+
    '.vocab-v25725-options{display:flex;flex-direction:column;gap:9px;margin-top:10px}.vocab-v25725-option{border:2px solid #e5e7eb;background:#fff;border-radius:14px;padding:12px 14px;text-align:left;font-size:17px;font-weight:900;color:#172554;font-family:inherit;cursor:pointer}'+
    '.vocab-v25725-option.correct{background:#dcfce7!important;border-color:#22c55e!important;color:#14532d!important}.vocab-v25725-option.wrong{background:#fee2e2!important;border-color:#ef4444!important;color:#7f1d1d!important}'+
    '.vocab-v25725-feedback{margin-top:12px;border-radius:14px;padding:12px;font-weight:900;text-align:center}.vocab-v25725-feedback.ok{background:#dcfce7;color:#14532d;border:2px solid #86efac}.vocab-v25725-feedback.warn{background:#fff7ed;color:#7c2d12;border:2px solid #fdba74}'+
  ...
  function patchDomainMenu(){
    ...
    card.onclick=function(ev){ ev.preventDefault(); window.openHibouVocabulaireV25725(); };
  }
  ...
  function renderMenu(){
    ...
    setInner(header('🧩 Vocabulaire','Choisis un entraînement.','Contenu chargé depuis le fichier JSON externe : '+count+' exercice(s) disponible(s).')+
      ...
      '<div class="vocab-v25725-menu">'+
      '<button class="vocab-v25725-card" type="button" onclick="startHibouVocabulaireV25725(\'famille\')"><div class="vocab-v25725-icon">🌳</div><div class="vocab-v25725-title">Familles de mots</div><div class="vocab-v25725-desc">Reconnaître les mots qui appartiennent à la même famille.</div></button>'+
      '<button class="vocab-v25725-card" type="button" onclick="startHibouVocabulaireV25725(\'synonyme\')"><div class="vocab-v25725-icon">🔁</div><div class="vocab-v25725-title">Synonymes</div><div class="vocab-v25725-desc">Trouver un mot de sens proche.</div></button>'+
      '<button class="vocab-v25725-card" type="button" onclick="startHibouVocabulaireV25725(\'contraire\')"><div class="vocab-v25725-icon">↔️</div><div class="vocab-v25725-title">Contraires</div><div class="vocab-v25725-desc">Trouver un mot de sens opposé.</div></button>'+
      '<button class="vocab-v25725-card" type="button" onclick="startHibouVocabulaireV25725(\'generique\')"><div class="vocab-v25725-icon">📦</div><div class="vocab-v25725-title">Mots génériques</div><div class="vocab-v25725-desc">Ranger des mots dans une catégorie.</div></button>'+
      '</div></section></div>');
  }

  window.openHibouVocabulaireV25725 = function(){
    injectStyle();
    showLoading();
    loadData(false).then(renderMenu);
  };

  window.startHibouVocabulaireV25725 = function(type){
  ...
  function renderNoData(type){
    setInner(header('🧩 Vocabulaire','Aucun exercice trouvé.','Ajoute des lignes actives dans data/vocabulaire.json pour cette activité.')+'<div class="fr-v2348-content"><section class="fr-v2348-panel"><div class="vocab-v25725-feedback warn">Aucun exercice disponible pour : '+esc(activityLabel(type))+'</div><div class="vocab-v25725-actions"><button class="vocab-v25725-btn secondary" onclick="openHibouVocabulaireV25725()">← Retour Vocabulaire</button></div></section></div>');
  }
  ...
  window.answerHibouVocabulaireV25725 = function(btn){
    var item=state.questions[state.index]; if(!item) return;
    var buttons=[].slice.call(document.querySelectorAll('.vocab-v25725-option'));
    ...
    var fb=$('vocabFeedbackV25725');
    var nextLabel = (state.index+1>=state.total) ? 'Voir le résultat' : 'Question suivante';
    if(fb) fb.innerHTML='<div class="vocab-v25725-feedback '+(ok?'ok':'warn')+'">'+(ok?'✅ Bravo !':'💡 Correction : '+esc(item.bonne_reponse||''))+(item.explication?'<br><small>'+esc(item.explication)+'</small>':'')+'</div><div class="vocab-v25725-actions"><button class="vocab-v25725-btn" onclick="nextHibouVocabulaireQuestionV25725()">'+nextLabel+'</button></div>';
  };
  window.nextHibouVocabulaireQuestionV25725 = function(){
    state.index++;
  ...
  function renderResult(){
    ...
    setInner(header('🧩 Vocabulaire','Entraînement terminé.','Le résultat est ajouté à Mon parcours récent.')+
      '<div class="fr-v2348-content"><section class="fr-v2348-panel"><div class="fr-v2348-result"><h3>Bravo, entraînement terminé !</h3><div class="fr-v2348-score-grid"><div class="fr-v2348-score-card">Score<b>'+state.score+' / '+total+'</b></div><div class="fr-v2348-score-card">Réussite<b>'+pct+'%</b></div></div><div class="fr-v2348-feedback">'+esc(label)+' · Source : JSON externe</div><div class="vocab-v25725-hints-used">💡 Aides utilisées : '+(state.hintsUsed||0)+'</div><div class="fr-v2348-actions"><button type="button" class="fr-v2348-action" onclick="startHibouVocabulaireV25725(\''+esc(state.activity)+'\')">🔁 Refaire</button><button type="button" class="fr-v2348-action secondary" onclick="openHibouVocabulaireV25725()">← Retour Vocabulaire</button><button type="button" class="fr-v2348-action secondary" onclick="renderFrenchDomainMenuV25720()">Menu Français</button></div></div></section></div>');
    track(label,total);
  }
